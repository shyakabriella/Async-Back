...
class ProgramController extends Controller
{
    ...
    private function getAcceptedPaidStudentCount(int $programId): int
    {
        return ProgramApplication::query()
            ->where('program_id', $programId)
    ...
    private function formatProgram(Program $program): array
    {
        ...
        $acceptedStudentsCount = $this->getAcceptedPaidStudentCount((int) $program->id);
        $currentBalance = $acceptedStudentsCount * $price;

        $data['price'] = $price;
        $data['shifts'] = $shifts;
        $data['curriculum'] = $curriculum;

        $data['accepted_students_count'] = $acceptedStudentsCount;
        $data['paid_students_count'] = $acceptedStudentsCount;
        $data['fee_per_student'] = $price;
        $data['current_balance'] = $currentBalance;
        $data['total_collected'] = $currentBalance;

        // keep students synced to paid/accepted students for frontend display
        $data['students'] = $acceptedStudentsCount;
